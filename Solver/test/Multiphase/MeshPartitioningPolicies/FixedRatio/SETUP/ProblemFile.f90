...
module UserDefinedMeshPartitioning
   ...
   pure integer function UserDefinedGetRegion(e) result(region)
      use SMConstants
      use ElementClass
      implicit none
      type(Element), intent(in) :: e

      if (all(e % storage % Q(1,:,:,:) < 1.0_RP - 1e-8_RP)) then
         region = 1
      else
         region = 2
      end if
   end function UserDefinedGetRegion
...
!        --------------------------------
!
            use MeshPartitioningPolicies, only: setPointerGetRegion
            use UserDefinedMeshPartitioning
            IMPLICIT NONE  

            call setPointerGetRegion(UserDefinedGetRegion)
