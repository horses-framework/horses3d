...
module RiemannSolvers_NSTPT
   ...
   abstract interface
      subroutine RiemannSolverFCN(QLeft, QRight, transportVelocityLeft, transportVelocityRight, nHat, t1, t2, flux)
         use SMConstants
         use PhysicsStorage_NSTPT
         real(kind=RP), intent(in)       :: QLeft(1:NCONS)
         real(kind=RP), intent(in)       :: QRight(1:NCONS)
         real(kind=RP), intent(in)       :: transportVelocityLeft(1:NDIM)
         real(kind=RP), intent(in)       :: transportVelocityRight(1:NDIM)
      ...
      subroutine AveragedStatesFCN(QLeft, QRight, transportVelocityL, transportVelocityR, pL, pR, invRhoL, invRhoR, flux)
         use SMConstants
         use PhysicsStorage_NSTPT
         real(kind=RP), intent(in)       :: QLeft(1:NCONS)
         real(kind=RP), intent(in)       :: QRight(1:NCONS)
         real(kind=RP), intent(in)       :: transportVelocityL(1:NDIM)
         real(kind=RP), intent(in)       :: transportVelocityR(1:NDIM)
      ...
      subroutine RiemannSolver_dFdQFCN(ql, qr, transportVelocityl, transportVelocityr, nHat, dfdq_num, side)
         use SMConstants
         use PhysicsStorage_NSTPT
...
!///////////////////////////////////////////////////////////////////////////////////////////
!
      subroutine BaseClass_RiemannSolver_dFdQ(ql,qr,transportVelocityLeft,transportVelocityRight,nHat,dfdq_num,side)
         implicit none
         !--------------------------------------------
         real(kind=RP), intent (in)  :: ql(NCONS)                 !<  Current solution on the left
         real(kind=RP), intent (in)  :: qr(NCONS)                 !<  Current solution on the right
      ...
      subroutine CentralRiemannSolver(QLeft, QRight, transportVelocityLeft, transportVelocityRight, nHat, t1, t2, flux)
         implicit none
         real(kind=RP), intent(in)       :: QLeft(1:NCONS)
         real(kind=RP), intent(in)       :: QRight(1:NCONS)
         real(kind=RP), intent(in)       :: transportVelocityLeft(1:NDIM)
...
!        ---------------
!
         real(kind=RP) :: rhoL, rhouL, rhovL, rhowL, rhoeL, cL, pL, rhoV2L
         real(kind=RP) :: rhoR, rhouR, rhovR, rhowR, rhoeR, cR, pR, rhoV2R
         real(kind=RP) :: invRhoL, invRhoR
         real(kind=RP) :: QLRot(NCONS), QRRot(NCONS), oflux(NCONS)
         real(kind=RP) :: transportVelocityL(NDIM), transportVelocityR(NDIM)
...
!        -----------------------------------------------------------------------------
         ...
         rhouL = QLeft(IRHOU) * nHat(IX) + QLeft(IRHOV) * nHat(IY) + QLeft(IRHOW) * nHat(IZ)
         rhovL = QLeft(IRHOU) * t1(IX)   + QLeft(IRHOV) * t1(IY)   + QLeft(IRHOW) * t1(IZ)
         rhowL = QLeft(IRHOU) * t2(IX)   + QLeft(IRHOV) * t2(IY)   + QLeft(IRHOW) * t2(IZ)

         rhouR = QRight(IRHOU) * nHat(IX) + QRight(IRHOV) * nHat(IY) + QRight(IRHOW) * nHat(IZ)
         rhovR = QRight(IRHOU) * t1(IX)   + QRight(IRHOV) * t1(IY)   + QRight(IRHOW) * t1(IZ)
         rhowR = QRight(IRHOU) * t2(IX)   + QRight(IRHOV) * t2(IY)   + QRight(IRHOW) * t2(IZ)

         rhoeL = QLeft(IRHOE) ; rhoeR = QRight(IRHOE)

         cL = QLeft(IC)
         cR = QRight(IC)

         transportVelocityL(IX) = transportVelocityLeft(IX) * nHat(IX) + transportVelocityLeft(IY) * nHat(IY) + transportVelocityLeft(IZ) * nHat(IZ)
         transportVelocityL(IY) = transportVelocityLeft(IX) * t1(IX)   + transportVelocityLeft(IY) * t1(IY)   + transportVelocityLeft(IZ) * t1(IZ)
         transportVelocityL(IZ) = transportVelocityLeft(IX) * t2(IX)   + transportVelocityLeft(IY) * t2(IY)   + transportVelocityLeft(IZ) * t2(IZ)

         transportVelocityR(IX) = transportVelocityRight(IX) * nHat(IX) + transportVelocityRight(IY) * nHat(IY) + transportVelocityRight(IZ) * nHat(IZ)
         transportVelocityR(IY) = transportVelocityRight(IX) * t1(IX)   + transportVelocityRight(IY) * t1(IY)   + transportVelocityRight(IZ) * t1(IZ)
         transportVelocityR(IZ) = transportVelocityRight(IX) * t2(IX)   + transportVelocityRight(IY) * t2(IY)   + transportVelocityRight(IZ) * t2(IZ)
...
!        ------------------------------------------------
         QLRot = (/ rhoL, rhouL, rhovL, rhowL, rhoeL, cL /)
         QRRot = (/ rhoR, rhouR, rhovR, rhowR, rhoeR, cR /)
         call AveragedStates(QLRot, QRRot, transportVelocityL, transportVelocityR, pL, pR, invRhoL, invRhoR, flux)
!
!        ************************************************
!        Return momentum equations to the cartesian frame
!        ************************************************
!
         flux(IRHOU:IRHOW) = nHat*flux(IRHOU) + t1*flux(IRHOV) + t2*flux(IRHOW)
...
!////////////////////////////////////////////////////////////////////////////////////////////
!
      subroutine StandardAverage(QLeft, QRight, transportVelocityL, transportVelocityR, pL, pR, invRhoL, invRhoR, flux)
!
!        *********************************************************************
...
!        ----------------
         ...
         flux(IC)    = 0.5_RP * ( QLeft(IC) * transportVelocityL(IX) + QRight(IC) * transportVelocityR(IX))
...
end module RiemannSolvers_NSTPT